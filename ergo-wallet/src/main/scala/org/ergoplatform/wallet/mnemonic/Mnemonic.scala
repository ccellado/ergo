...
object Mnemonic {
  ...
  def toSeed(mnemonic: SecretString, passOpt: Option[SecretString] = None): Array[Byte] = {
    val normalizedMnemonic = normalize(mnemonic.getData().toString, NFKD).toCharArray
    val normalizedPass = normalize(("mnemonic".toCharArray ++ passOpt.fold("".toCharArray())(_.getData())).toString, NFKD)
